...
function scene:create( event )
	...
	function pop( event )
		...
		local title = display.newText(popGroup, wallPaper[i].name, display.contentCenterX, display.contentHeight*0.37, "Content/font/ONE Mobile POP.ttf")
		...
		local picture = display.newImageRect(popGroup, wallPaper[i].image, 150, 150)
		...
		local buyingText = display.newText(popGroup, "         " .. wallPaper[i].price ..  "    구매", display.contentWidth*0.78, display.contentHeight*0.6, "Content/font/ONE Mobile POP.ttf")
	    ...
	    local function consume( event )
			...
			-- 시럽 보유 카운트--
			wallCnt[i] = wallCnt[i] + 1

	   		deleteBeforeCnt(i)
	    	displayIngreCnt(i)

			print(wallPaper[i].name .."시럽 산 뒤에" .. wallCnt[i])
