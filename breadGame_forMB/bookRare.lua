...
-- local function makeRareBook(js, 64) -- 전체 도감
	local function makeRareBook()
		...
		for i = 1, 32 do
			...
			Bimage = UBreadInfo[index1].breads[index2].image
			Bname = UBreadInfo[index1].breads[index2].name
			if i % 3 == 1 then
				jul = jul + 0.33
			end
			if lock ~= 0 then
